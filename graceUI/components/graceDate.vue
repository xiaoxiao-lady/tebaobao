<template name="graceDate">
	<view class="grace-date" v-if="show">
		...
		<view class="grace-date-days">
			...
			<view 
			v-for="(itemList, index2) in days" :key="index2"
			:class="['items', currentDay == (cYear+'-'+cMonth+'-'+itemList.date) ? 'current' : '',itemList.isselect?'current':'']" 
			 @tap="gracClick(itemList.date,itemList.isselect)" 
			 :data-date="cYear+'-'+cMonth+'-'+itemList.date"
			  :style="{'background' : itemList == '' ? 'none' : ''}">
				<view class="grace-date-day" >{{itemList.date? itemList.date: ''}} </view>
				<view class="grace-date-nl">{{itemList.nl?itemList.nl: ''}}</view>
			</view>
		</view>
		<view v-if="isTime">
...
export default {
	...
	methods: {
		...
		// (item.date && !item.isselect)  ?chooseDate(item.date) : ''
		//  把当月签到的天数 添加 isselect 字段标记出来 
		isSing(){
			for(var i =0;i < this.sing_arr.length;i++){
				for(var k =0;k < this.days.length;k++){
					 if(this.sing_arr[i] ==(this.cYear + '-'+ this.cMonth+'-'+ this.days[k].date)){
						this.days[k].isselect = true
					 }	
				}
			}
		...
		chooseDate: function (index) {
			...
			var str = arr[0]+'-'+ this.cMonth+ '-' + index
			
			this.$emit('changeDate',this.currentDay,str,this.days);
		},
		submit : function(){
			var arr = this.currentDay.split('-');
	...
	watch:{
		// sing_arr 代表当月所有签到的天数
		sing_arr(val){
			this.sing_arr = val
			this.isSing()
			console.log(this.sing_arr,456)
		}
	}
}
</script>
